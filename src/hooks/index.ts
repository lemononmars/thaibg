...
export async function handle({
	...
	event: RequestEvent;
	resolve: (request: RequestEvent) => Promise<Response>;
}) {
	const sbToken = event.request.headers.get('Cookie')
		? parse(event.request.headers.get('Cookie'))[COOKIE_NAME]
		: '';
	...
	if (event.request.method === 'POST' && new URL(event.request.url).pathname === API_AUTH) {
		...
		if (authChangeEvent === 'SIGNED_IN') {
			...
			const cookieHeader = await serialize(COOKIE_NAME, 'deleted', {
				...COOKIE_OPTIONS,
				maxAge: 0
			});
