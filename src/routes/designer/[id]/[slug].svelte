...
<script lang="ts">
   ...
   import {DIR_IMAGE, URL_BLANK_BG_IMAGE, URL_BLANK_PERSON_IMAGE} from '$lib/constants'
   ...
   onMount(async ()=>{
      ...
      designerData.Designer_picture = DIR_IMAGE + '/designer/' + (designerData.Designer_picture || URL_BLANK_PERSON_IMAGE)
   })
      ...
      {:else}
         {#if boardgameData}
            <div class="flex flex-col lg:flex-row lg:gap-4 w-full p-8 border-2 shadow-lg rounded-xl">
               <img src="{designerData.Designer_picture}" alt="image of {designerData.Designer_name}" class="w-72 mask mask-hexagon-2"/>
               <div>
