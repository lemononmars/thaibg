...
<script lang="ts">
   ...
   $: contentsFiltered = contents.filter((c)=> 
         (optionType == 0
            || c.Content_type?.toLowerCase() === ContentTypeFilter[optionType]
         )
            &&
         (optionMedia == 0
            || c.Content_media?.toLowerCase() === ContentMediaFilter[optionMedia]
         )
            && 
         (searchString === ''
...
<div class="flex flex-col justify-center items-center relative mx-auto">
   ...
   {#if contentsSorted?.length == 0}
      <div>
         Try <div class="btn btn-outline" on:click={resetSearch}>resetting</div> your search query.
      </div>
   {/if}
</div>