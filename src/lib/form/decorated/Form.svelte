...
<script lang="ts">
	...
	const formIsValid = derived(
		formState,
		($state) =>
			Object.keys($state).length > 0 && Object.entries($state).every(([_, field]) => field.isValid)
	);

	// @ts-ignore
	const getFormState = (): FormStateNormalized =>
		Object.entries($formState).reduce((acc, [key, val]) => {
	...
	setContext(formKey, {
		setValueAndValidity: (fieldName, value, isValid) =>
			formState.update((prevState) => ({ ...prevState, [fieldName]: { value, isValid } }))
	});
</script>
