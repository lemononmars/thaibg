<script lang=ts>
   ...
   $: filteredOptions = selectOptions.filter(s =>
      !selects.includes(s) && s.includes(searchString)
   )  
...
<div class="flex flex-col mx-auto">
   <div class="dropdown mb-4">
      ...
      >
         ...
         {:else}
            <li><div class="btn btn-outline btn-info" on:click={()=>select(searchString)}>Add "{searchString}"</div></li>
         {/each}
      </ul>
   </div>
   {#each selects as s}
